...
这是为了防止选手连进WIFI直接做ARP等中间人的攻击方法直接获得到包含flag的通信流量，因为主要希望考察点是：即使是不在目标通信干路上，也能获得目标设备的MQTT通信内容，并且将恶意数据通过broker带向内网设备，这也是MQTT这种消息队列协议的特性。另外可以看到MQTT服务是连接到了一个域名上，是我花了一块钱买了一年的域名：
